...
setwd("/media/deeksha/e/Deeksha/Dropbox/Coursework/DataVisualization")
...
p1<-p1+ggtitle("Rating of movies vs budget")+scale_colour_brewer(palette="Set1")
...
p1<-ggplot(movies,aes(genre))+geom_histogram(fill="blue",color="black",binwidth = 0.5)
...
p1+ggtitle("Rating of different genre movies and budget")
...
p1<-p1+theme(title=element_text(size=15),legend.title=element_text(size=15),legend.text=element_text(size=12),axis.title= element_text(size=15,face="bold"),axis.text = element_text(colour="black",size=14,face="bold"))
p1+ggtitle("Index Prices over Time")
